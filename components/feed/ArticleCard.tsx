      ...
      <article className="h-full flex flex-col gap-4">
        ...
        <div className="relative aspect-[16/9] rounded-2xl overflow-hidden bg-neutral-100">
          ...
          <div className="absolute top-3 right-3">
            <span className="text-[10px] font-sans font-semibold uppercase tracking-widest px-2.5 py-1 rounded-full bg-ns-black/70 text-white backdrop-blur-sm">
        ...
        <div className="flex flex-col gap-2.5 flex-1">
          ...
          {/* Title */}
          <h2 className="font-heading font-bold text-lg leading-snug text-ns-black group-hover:text-ns-blue transition-colors line-clamp-2">
            {article.title}
          </h2>

          {/* Summary */}
          <p className="text-sm leading-6 text-neutral-500 line-clamp-2 font-sans">
            {article.summary}
          </p>
          ...
          {article.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-auto pt-1">
              {article.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag.id}
                  className="text-[10px] font-sans text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded-full"
                >
                  {tag.name}
                </span>
              ))}
